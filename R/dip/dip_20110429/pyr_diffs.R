...
*(1-massloss[harvest==6])
      mean<-(tapply(orig_cTIC$L[harvest==6], type[harvest==6], mean)-tapply(orig_cTIC$L[harvest==0], type[harvest==0], mean))
*0.01 / (tapply(massloss[harvest==6], type[harvest==6], mean) - tapply(massloss[harvest==0], type[harvest==0], mean))
      sds<-sqrt(tapply(orig_cTIC$L[harvest==6], type[harvest==6], sd)^2 + tapply(orig_cTIC$L[harvest==0], type[harvest==0], sd)^2)
 / (tapply(massloss[harvest==6], type[harvest==6], mean) - tapply(massloss[harvest==0], type[harvest==0], mean))
error<-CI2(sds, 4, 0.05)
