...
cor.test
plot(orig_cTIC$Ph, orig_cTIC$L, type="n")
    for (i in 1:length(typlev))
      for (j in 1:length(harlev))
      points(orig_cTIC$Ph[harvest==harlev[j] & type==typlev[i]], orig_cTIC$L[harvest==harlev[j] & type==typlev[i]], col=colscale[j], pch=pch[i], cex=1)
